 ...
 *
 * Gli stream sono da intendersi come sequenze casuali diverse a cui attingere
 * per modellare componenti casuali del sistema diversi
 *
 ...
 * values dependent on the value of x (this is probably done to reduce overlapping between stream).  
...
// vector that contains the sequences of numbers generated by the system   
static long seed[STREAMS] = {DEFAULT};  /* current state of each stream   */
// we use one stream at time. Streams are probably introduced to reduce
// the number of repetitions, so stream are really a way to control randomness and reproducibility
...
// imposta i seed di tutti gli stream
// il valore passato è il seed del primo stream
// a partire dal quale vengono calcolati tutti gli altri
...
// imposta il seed di uno stream solo
   void PutSeed(long x)
/* ---------------------------------------------------------------
